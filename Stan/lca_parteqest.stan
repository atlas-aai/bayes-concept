...
parameters {
  ...
  // item-level deviations from mean
  real intercept_dev[I];
  real<lower=-mean_maineffect> maineffect_dev[I];
  
  // deviation variances
  real<lower=0> intercept_sd;
  real<lower=0> maineffect_sd;
}
transformed parameters {
  ...
  for (i in 1:I) {
    intercept[i] = mean_intercept + intercept_dev[i];
    maineffect[i] = mean_maineffect + maineffect_dev[i];
...
model{
  ...
  intercept_dev ~ normal(0, intercept_sd);
  maineffect_dev ~ normal(0, maineffect_sd);
  intercept_sd ~ student_t(3, 0, 1) T[0,];
  maineffect_sd ~ student_t(3, 0, 1) T[0,];
